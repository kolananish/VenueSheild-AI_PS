...
const useResponsiveDimension = (
  ...
  useEffect(() => {
    ...
    const calc = () => {
      ...
      if (
        w <= 480 &&
        config[`mobile${capitalizedKey}` as keyof GradualBlurConfig]
      )
        v = config[
          `mobile${capitalizedKey}` as keyof GradualBlurConfig
        ] as string;
      else if (
        w <= 768 &&
        config[`tablet${capitalizedKey}` as keyof GradualBlurConfig]
      )
        v = config[
          `tablet${capitalizedKey}` as keyof GradualBlurConfig
        ] as string;
      else if (
        w <= 1024 &&
        config[`desktop${capitalizedKey}` as keyof GradualBlurConfig]
      )
        v = config[
          `desktop${capitalizedKey}` as keyof GradualBlurConfig
        ] as string;
      setValue(v);
    };
...
function GradualBlur(props: GradualBlurConfig) {
  ...
  const blurDivs = useMemo(() => {
    ...
    const curveFunc =
      CURVE_FUNCTIONS[config.curve || "linear"] || CURVE_FUNCTIONS.linear;
  ...
  useEffect(() => {
    if (isVisible && animated === "scroll" && onAnimationComplete) {
      const ms = Number.parseFloat(duration || "0.3s") * 1000;
